...
from transformers import AutoModelForCausalLM, AutoModelForSeq2SeqLM, get_linear_schedule_with_warmup
from transformers import (
    ProphetNetEncoder,
    ProphetNetDecoder,
    ProphetNetPreTrainedModel,
    ProphetNetModel as OldProphetNetModel,
    ProphetNetForConditionalGeneration as OldProphetNetForConditionalGeneration)
...
class LightModel(pl.LightningModule):
    def __init__(self, args):
        ...
        if self.args.model_type is not None:
            model_cls, tokenizer_cls = MODEL_CLASSES[self.args.model_type]
        else:
            model_cls, tokenizer_cls = AutoModelForSeq2SeqLM, AutoTokenizer
        self.model = model_cls.from_pretrained(self.args.model_path)
        self.tokenizer = tokenizer_cls.from_pretrained(self.args.model_path)
    ...
    def training_step(self, batch, batch_idx):
        ...
        is_prob = True if hasattr(self.model, 'generator') else False
        loss = ce_loss(logits, batch['labels'], is_prob=is_prob, eps=self.args.ls_eps)
        if self.args.rdrop:
    ...
    def ids2text(self, ids):
        ...
        text = [s.replace(' ', '').lower() for s in text]
        return text
    ...
    def validation_step(self, batch, batch_idx):
        ...
        labels = self.ids2text(
            batch['labels'].masked_fill_(batch['labels'] == -100, self.tokenizer.pad_token_id)
        )
        rouge_score = compute_rouge(labels, pred)
        ret.update(rouge_score)
        bleu_score = compute_bleu(labels, pred)
        ret['bleu'] = bleu_score
        return ret
...
class T5Copy(T5ForConditionalGeneration):
    ...
    def _prepare_encoder_decoder_kwargs_for_generation(
        ...
        src = encoder_kwargs.pop('src')
        model_kwargs["encoder_outputs"] = encoder(**encoder_kwargs)
        model_kwargs['src'] = src
        return model_kwargs
...
class ProphetNetModel(OldProphetNetModel):
    def __init__(self, config):
        super(ProphetNetPreTrainedModel, self).__init__(config)
        self.word_embeddings = nn.Embedding(config.vocab_size, config.hidden_size, padding_idx=config.pad_token_id)

        encoder_config = copy.deepcopy(config)
        encoder_config.is_encoder_decoder = False
        encoder_config.use_cache = False
        encoder_config.max_position_embeddings = config.encoder_max_position_embeddings
        self.encoder = ProphetNetEncoder(encoder_config, self.word_embeddings)

        decoder_config = copy.deepcopy(config)
        decoder_config.is_decoder = True
        decoder_config.is_encoder_decoder = False
        decoder_config.max_position_embeddings = config.decoder_max_position_embeddings
...
class ProphetNetForConditionalGeneration(OldProphetNetForConditionalGeneration):
    def __init__(self, config):
        super(ProphetNetPreTrainedModel, self).__init__(config)
...
MODEL_CLASSES = {
    ...
    'prophet': [ProphetNetForConditionalGeneration, CPTTokenizer]
}
